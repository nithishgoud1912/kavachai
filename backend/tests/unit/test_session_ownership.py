"""
KavachAI — Unit Tests for Person 3: Session Ownership & Tool Definitions
Task 3.5

Tests:
    test_document_model_has_session_id
    test_dataset_model_has_session_id
    test_tag_chunks_includes_session_id_in_metadata
    test_tag_chunks_default_session_id_is_empty
    test_document_retrieval_filters_by_session
    test_tool_validates_dataset_ownership
"""
...
# ---------------------------------------------------------------------------
# Task 3.1 — DB model column tests
# ---------------------------------------------------------------------------

class TestDatabaseModels:
    """Verify that session_id columns were added correctly (Task 3.1)."""
...
# ---------------------------------------------------------------------------
# Task 3.2 — tag_chunks session_id tests
# ---------------------------------------------------------------------------
...
class TestTagChunks:
    """Verify session_id is written into ChromaDB metadata (Task 3.2)."""
...
# ---------------------------------------------------------------------------
# Task 3.3 — document_agent session filtering tests
# ---------------------------------------------------------------------------

class TestDocumentRetrievalSessionFilter:
    """Verify _build_where_filter handles session_id correctly (Task 3.3)."""
    ...
    def test_session_id_filter_builds_in_clause(self):
        ...
        # ChromaDB $in clause must include the session AND shared values
...
# ---------------------------------------------------------------------------
# Task 3.4 — Tool dataset ownership validation test
# ---------------------------------------------------------------------------

class TestToolDatasetOwnership:
    """Verify analyze_operational_data rejects cross-session access (Task 3.4)."""
    ...
    async def test_tool_validates_dataset_ownership_rejects_cross_session(self):
        ...
        from app.langchain.tools import analyze_operational_data

        mock_config = {"configurable": {"session_id": "sess-current"}}

        # Simulate DB returning no result (different session or not found)
        mock_db = AsyncMock()
    ...
    async def test_tool_validates_dataset_ownership_allows_shared(self):
        ...
        # Simulate DB returning a shared dataset (session_id is None)
