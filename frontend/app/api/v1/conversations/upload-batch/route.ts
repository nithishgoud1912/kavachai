...
export async function POST(req: Request) {
  ...
  // Check if backend has batch upload endpoint
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 2000);
    const backendRes = await fetch(`${BACKEND_URL}/conversations/upload-batch`, {
      method: "POST",
      headers: {
    ...
    // If backend doesn't support batch upload yet, handle locally
  }
  ...
  try {
    ...
    const uploaded = await Promise.all(
      allFiles.map(async (file, idx) => {
        const filename = (file as File).name || `file_${idx + 1}`;
        ...
        if (isDocument) {
          if (ext === ".txt" || ext === ".csv" || ext === ".md" || ext === ".json") {
            ...
            if (!extractedText) {
              extractedText = `[Ingested PDF: ${filename}, ${relativePath}]`;
            }
          } else {
            extractedText = `[Attached technical document: ${filename}, ${relativePath}]`;
          }
        }

        // Store in mock storage for serving
